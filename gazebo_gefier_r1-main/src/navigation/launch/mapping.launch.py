...
def generate_launch_description():
    ...
    pkg_hv_bot = get_package_share_directory('navigation')
    gazebo = get_package_share_directory('description_new')

    gazebo_models_path, ignore_last_dir = os.path.split(gazebo)
    ...
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        arguments=['-d', PathJoinSubstitution([pkg_hv_bot, 'rviz', LaunchConfiguration('rviz_config')])],
    ...
    interactive_marker_twist_server_node = Node(
        ...
        parameters=[
            ...
            {'use_sim_time': LaunchConfiguration('use_sim_time')} # <--- Added this!
        ],
        output='screen',
    )
